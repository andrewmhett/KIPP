        ...
        if int(datetime.strftime(datetime.now(), format("%H"))) >= 10:
            ...
            for stock in stocks:
                if len(stock) > 0:
                    ...
                    if new_percent > 0:
                        new_percent = max(random_perc / 10,
                                          min(new_percent, random_perc))
                    else:
                        new_percent = max(-random_perc,
                                          min(new_percent, -random_perc / 10))
...
async def on_guild_join(server):
    ...
    serverinfo[server].task = client.loop.create_task(
        serverinfo[server].update_loop())
