...
hash_value = int(
    ("04" + (hashlib.sha256(str(nonce).encode()).hexdigest()[2:])), 16)
...
async def MINE(message, message2, serverinfo, playerinfo):
    global hash_value
    global salt
    guess = int(hashlib.sha256(
        str(int(message2.split("|")[1]) + salt).encode()).hexdigest(), 16)
    if guess <= hash_value:
...
async def TRANSFER(message, message2, serverinfo, playerinfo):
    try:
        ...
        receiver = message.guild.get_member_named(
    ...
    if int(amount) <= int(playerinfo[patron].GET_KIPPCOINS()) and int(amount) > 0:
        ...
        emb = discord.Embed(title="Transfer", description="Transferred `{0}` KIPPCOINS to **{1}**'s account.\n You now have `{2}` KIPPCOINS.".format(
            amount, str(receiver), int(playerinfo[patron].GET_KIPPCOINS())), colour=EMBEDCOLOR)
...
async def GAMBLEGAME(message, message2, serverinfo, playerinfo):
    if message2.split('|')[1] == "SOLO":
        ...
        emb = discord.Embed(title="Solo GambleGame",
                            description="Use `!SELECT|color` to choose a color:\n`Red`\n`Blue`\n`Green`\n`Black`", colour=EMBEDCOLOR)
        emb.set_footer(text=get_footer())
        message1 = await message.channel.send(embed=emb)
        playerinfo[message.author].gamblemessage = message1
    else:
        try:
            opponent = message.guild.get_member_named(
        ...
        if str(opponent.status).upper != "OFFLINE" and str(opponent.status).upper != "IDLE":
            ...
            desc = "{0}, you have been challenged to a GambleGame by {1}.\n**!ACCEPT** or **!DECLINE**\n{2} may **!CANCEL**".format(
                opponent.mention, message.author.mention, message.author.mention)
...
async def INVENTORY(message, message2, serverinfo, playerinfo):
    items = subprocess.Popen(["sudo", "cat", KIPP_DIR + "/C++/SHOP_ITEMS.txt"],
    ...
    for item in items:
        if len(item) > 0:
            ...
            if playerinfo[message.author].HAS_ITEM(item_index):
                if "PLAQUE" in item:
                    plaques.append(item.split("KIPP YEAR ")
                                   [1].split(" PLAQUE")[0])
                else:
    ...
    if len(plaques) > 0:
        ...
        for plaque_num in plaques:
            roman_numeral_row = "YEAR {0}".format(
                to_roman_numeral(int(plaque_num)))
            ...
            plaque_str += "```\n+--------------------+\n|========KIPP========|\n|{0}|\n|=December 1st, {1}=|\n+--------------------+```".format(
                roman_numeral_row, str(2017 + int(plaque_num)))
...
async def SHOP(message, message2, serverinfo, playerinfo):
    ...
    em.description = "Current balance: `{0} KC`\nAM stands for AUTO-MINER\n```#  NAME                      PRICE\n".format(
        playerinfo[message.author].GET_KIPPCOINS())
    item_index = 0
    for item in items:
        if len(item) > 0:
            item_index += 1
    ...
    if len(in_budget) > 0:
        ...
        for item in in_budget:
            em.description += "\n{0}".format(item[1]) + "  "
            em.description += "{0}".format(item[0]) + \
                (" " * (26 - len(item[0])))
            if len(str(item_dict[item[0]])) > 5:
                em.description += "10^{0}".format(
                    len(str(item_dict[item[0]])) - 1)
            else:
...
async def BALANCE(message, message2, serverinfo, playerinfo):
    em = discord.Embed(title="KIPPCOIN Balance", description="Balance: `{0} KC`".format(
        playerinfo[message.author].GET_KIPPCOINS()), color=EMBEDCOLOR)
    await message.channel.send(embed=em)


async def STOCKS(message, message2, serverinfo, playerinfo):
    em = discord.Embed(title="KIPPCOIN Stocks", color=EMBEDCOLOR)
    markets = subprocess.Popen(["sudo", "-E", KIPP_DIR + "/C++/STOCKS_IO", "r", "a"],
    ...
    stock_deltas = subprocess.Popen(["sudo", "cat", KIPP_DIR + "/STOCKS.txt"],
                                    stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].decode().split("\n")
    market_change_map = {}
        ...
        if len(market) > 0:
            ...
            try:
                percent_change = round(
                    100 * (int(kc_change) / (int(per_share_price) - int(kc_change))), 1)
            except ZeroDivisionError:
                percent_change = 0
            change_string = str(kc_change) + (" " * (6 - len(str(kc_change)))) + \
                "(" + ('+' if percent_change >= 0 else '') + \
                str(percent_change) + "%)"
    ...
    shares = subprocess.Popen(["sudo", "-E", KIPP_DIR + "/C++/SHARES_IO", "r", str(message.author.id), "a"],
                              stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].decode().split("\n")
    shares_string = ""
    has_shares = False
    for share in shares:
            ...
            if market.split(":")[0] == message2.split("|")[1] or market.split(":")[0].replace("_", "") == message2.split("|")[1]:
                ...
                if num_purchasing <= int(market.split(":")[1].split(" ")[1]):
                    if price_per_share * num_purchasing <= playerinfo[message.author].GET_KIPPCOINS():
                        num_shares_held = int(subprocess.Popen(["sudo", "-E", KIPP_DIR + "/C++/SHARES_IO", "r", str(
                            message.author.id), market.split(":")[0]], stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].decode())
                        os.system("sudo -E {0}/C++/SHARES_IO wn {1} {2} {3}".format(
                            KIPP_DIR, message.author.id, market.split(":")[0], num_purchasing + num_shares_held))
                        os.system("sudo -E {0}/C++/STOCKS_IO wn {1} {2}".format(KIPP_DIR, market.split(
                            ":")[0], int(market.split(":")[1].split(" ")[1]) - num_purchasing))
                        playerinfo[message.author].GIVE_KIPPCOINS(
                            -1 * num_purchasing * (int(market.split(":")[1].split(" ")[2])))
...
async def SELLSHARES(message, message2, serverinfo, playerinfo):
    ...
    for market in markets:
        if len(market) > 0:
            market_value_map[market.split(":")[0]] = [int(market.split(":")[1].split(" ")[
                2]), int(market.split(":")[1].split(" ")[1])]
    market_found = False
    for share in shares:
        if len(share) > 0:
            if share.split(":")[0] == message2.split("|")[1] or share.split(":")[0].replace("_", "") == message2.split("|")[1]:
                ...
                if num_selling <= num_shares_held:
                    ...
                    os.system("sudo -E {0}/C++/SHARES_IO wn {1} {2} {3}".format(
                        KIPP_DIR, message.author.id, market.split(":")[0], num_shares_held - num_selling))
                    os.system("sudo -E {0}/C++/STOCKS_IO wn {1} {2}".format(
                        KIPP_DIR, market, market_value_map[market][1] + num_selling))
                    playerinfo[message.author].GIVE_KIPPCOINS(
                        num_selling * market_value_map[market][0])
...
async def SHARES(message, message2, serverinfo, playerinfo):
    ...
    if has_shares:
        ...
        for share in shares:
            if len(share) > 0:
                if int(share.split(": ")[1]) > 0:
                    value_string = "{0}  {1}".format(
                        share.split(":")[0], share.split(": ")[1])
                    shares_string += "\n" + value_string
                    shares_string += " " * (15 - len(value_string))
                    shares_string += str(market_value_map[share.split(":")[
                                         0]] * int(share.split(": ")[1]))
        shares_string += "```\nSee `!HELP|SELLSHARES`"
    else:
    ...
    for market in markets:
        if len(market) > 0:
            price_per_share = int(market.split(":")[1].split(" ")[2])
            within_budget = int(
                playerinfo[message.author].GET_KIPPCOINS() / price_per_share)
            if within_budget > 0:
                can_afford = True
                afford_string += "\n`{0} share{1} of {2}`".format(
                    within_budget, ("s" if within_budget != 1 else ""), market.split(":")[0])
    if not can_afford:
        afford_string = "You cannot afford any shares."
    em.add_field(name="Shares You can Afford",
                 value=afford_string, inline=False)
    await message.channel.send(embed=em)
...
async def LEADERBOARD(message, message2, serverinfo, playerinfo):
    ...
    for member in message.guild.members:
        if not member.bot:
            ...
            for share in shares:
                if len(share) > 0:
                    net_worth += market_value_map[share.split(
                        ":")[0]] * int(share.split(": ")[1])
            net_worth_dict[member.id] = [str(member), net_worth]
    await message.channel.send("Sorting...")
    net_worth_pairs = sorted(net_worth_dict.values(),
    ...
    for pair in net_worth_pairs:
        if position < 10:
            ...
            if len(display_name) > 17:
                name_sections = ["#".join(display_name.split(
                    "#")[0:-1]), display_name.split("#")[-1]]
...
async def TRENDS(message, message2, serverinfo, playerinfo):
    ...
    trends = subprocess.Popen(["sudo", "cat", KIPP_DIR + "/TRENDS.txt"], stdout=subprocess.PIPE,
    ...
    for trend in trends:
        ...
        if len(trend) > 0:
            ...
            plt.plot(x_values, np.array(y_values),
                     label=trend.split(":")[0].replace("_", ""))
    plt.legend(loc=2)
    plt.xlabel("Days")
...
KIPC("!MINE", "This command stacks all of your KIPPCOIN multipliers and adds that amount of KIPPCOINS to your account. This command will not return any message\n!MINE|guess number",
     MINE, [int])
KIPC("!TRANSFER", "This command will transfer a given amount of KIPPCOINS from your account to another account\n!TRANSFER|amount|receiver",
     TRANSFER, [int, str])
KIPC("!GAMBLEGAME", "This command will start either a solo or multiplayer gambling game involving KIPPCOINS\n!GAMBLEGAME|'SOLO' or opponent user",
     GAMBLEGAME, [str])
KIPC("!SHOP", "Opens the KIPPCOIN shop\n!SHOP", SHOP, [])
KIPC("!BUY", "Purchases an item from the shop\n!BUY|item number", BUY, [int])
KIPC("!INVENTORY", "Displays your inventory\n!INVENTORY", INVENTORY, [])
KIPC("!BALANCE", "Displays your KIPPCOIN balance\n!BALANCE", BALANCE, [])
KIPC("!STOCKS", "Displays relevant stock market information\n!STOCKS", STOCKS, [])
KIPC("!STONKS", "An alias for `!STOCKS`\n!STONKS", STONKS, [])
KIPC("!SHARES", "Displays information about the stock shares you own\n!SHARES", SHARES, [])
KIPC("!BUYSHARES", "Buys a specified number of shares from a specified market\n!BUYSHARES|market name|number of shares",
     BUYSHARES, [str, int])
KIPC("!SELLSHARES", "Sells a specified number of shares from a specified market\n!SELLSHARES|market name|number of shares",
     SELLSHARES, [str, int])
KIPC("!LEADERBOARD",
     "Displays a KIPPCOIN net worth leaderboard\n!LEADERBOARD", LEADERBOARD, [])
KIPC("!TRENDS", "Displays an image of a graph showing the trends for the past 10 days of all stocks\n!TRENDS", TRENDS, [])
