            ...
            if message2.split("|")[1].startswith("PLAYLIST:")==False:
                                ...
                                if ((music3[0]).upper() == "!MUSIC"):
                                    ...
                                    if "user" not in music4:
                                        ...
                                        if serverinfo[message.server].playlist != None:
                                            serverinfo[message.server].queue=serverinfo[message.server].queue[:-1]
                                        ...
                                        if serverinfo[message.server].mHandler != None:
                                            if len(serverinfo[message.server].queue)>1:
                                                if serverinfo[message.server].playlist != None:
                                                    await client.send_message(message.channel, "Song added to queue. #"+str(len(serverinfo[message.server].queue)-2))
                                                else:
                ...
                if serverinfo[message.server].search_server_configs(message2.split("|")[1]) != None:
                    if len(serverinfo[message.server].search_server_configs(message2.split("|")[1])[0][1:])>0:
                        ...
                        await client.send_message(message.channel, "There are no songs in the playlist named `{0}`. You may append songs to this playlist with **!APPENDPLAYLIST**.".format(message2.split("PLAYLIST:")[1]))
                else:
                    await client.send_message(message.channel, "There is no playlist named `{0}`. Use **!PLAYLISTS** to see a list of all playlists in this server.".format(message2.split("PLAYLIST:")[1]))
        except Exception as err:
            serverinfo[message.server].loading = False
            await client.send_message(message.channel, err)
            raise
