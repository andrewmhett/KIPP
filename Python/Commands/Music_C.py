...
async def NEWPLAYLIST(message, message2, serverinfo, playerinfo):
    ...
    if serverinfo[message.guild].search_server_configs("PLAYLIST:{0}".format(name)) == None:
        serverinfo[message.guild].add_server_config(
            ["PLAYLIST:{0}".format(name)])
...
async def DELETEPLAYLIST(message, message2, serverinfo, playerinfo):
    name = message2.split("|")[1]
    if serverinfo[message.guild].search_server_configs("PLAYLIST:{0}".format(name)) != None:
        serverinfo[message.guild].change_server_config(
            "PLAYLIST:{0}".format(name), "")
        counter = 0
        new_queue = []
...
async def RENAMEPLAYLIST(message, message2, serverinfo, playerinfo):
    ...
    if serverinfo[message.guild].search_server_configs("PLAYLIST:{0}".format(name)) != None:
        ...
        for link in serverinfo[message.guild].search_server_configs("PLAYLIST:{0}".format(name))[0][1:]:
            line.append(link)
        serverinfo[message.guild].change_server_config(
            "PLAYLIST:{0}".format(name), line)
        counter = 0
        for song in serverinfo[message.guild].queue:
            if song == "PLAYLIST: {0}".format(name):
                serverinfo[message.guild].queue[counter] = "PLAYLIST: {0}".format(
                    new_name)
            counter += 1
...
async def APPENDPLAYLIST(message, message2, serverinfo, playerinfo):
    ...
    if serverinfo[message.guild].search_server_configs("PLAYLIST:{0}".format(name)) != None:
        ...
        if len(serverinfo[message.guild].search_server_configs("PLAYLIST:{0}".format(name))[0]) > 1:
            arr = serverinfo[message.guild].search_server_configs(
                "PLAYLIST:{0}".format(name))[0][1:]
        else:
            arr = []
        if "user" in music4:
            return
        ...
        if "list" in music4:
            ...
            for song in api.get_playlist_items(playlist_id=tag, count=None).items:
                arr.append(
                    "https://www.youtube.com/watch?v={0}".format(song.snippet.resourceId.videoId))
                counter += 1
        else:
            single = True
            arr.append(music4)
...
async def PLAYLISTS(message, message2, serverinfo, playerinfo):
    ...
    if serverinfo[message.guild].search_server_configs("PLAYLIST") != None:
        ...
        embed = discord.Embed(title="Playlists", color=EMBEDCOLOR,
                              description="There are no playlists in this server")
        await message.channel.send(embed=embed)
                ...
                if "playlist" not in music4:
                    ...
                    if "channel" not in music4:
                        ...
                        if serverinfo[message.guild].playlist != None:
                            ...
                            serverinfo[message.guild].queue.append(
                                "PLAYLIST: {0}".format(serverinfo[message.guild].playlist))
                        else:
                            add_to_queue(message.guild, music4, serverinfo)
                        if serverinfo[message.guild].mHandler != None:
                    ...
                    for song in api.get_playlist_items(playlist_id=music4.split("playlist?list=")[1], count=None).items:
                        serverinfo[message.guild].queue.append(
                            [song.snippet.title, "https://www.youtube.com/watch?v={0}".format(song.snippet.resourceId.videoId)])
                        counter += 1
                ...
                if serverinfo[message.guild].playlist == None:
                    if serverinfo[message.guild].search_server_configs(message2.split("|")[1]) != None:
                        if len(serverinfo[message.guild].search_server_configs(message2.split("|")[1])[0][1:]) > 0:
                            await join_voice_channel(message, serverinfo)
                            serverinfo[message.guild].musicchannel = message.channel
                            serverinfo[message.guild].playlist = message2.split('PLAYLIST:')[
                                1]
                            serverinfo[message.guild].queue.append(
                                "PLAYLIST: {0}".format(message2.split("PLAYLIST:")[1]))
                        else:
...
async def QUEUE(message, message2, serverinfo, playerinfo):
    ...
    if len(serverinfo[message.guild].queue) > 1:
        ...
        if len(serverinfo[message.guild].queue) - i > 1:
            queuelist += ("\n`...and {0} more songs`".format(len(serverinfo[message.guild].queue) - i) if len(
                serverinfo[message.guild].queue) - i > 2 else "\n`...and 1 more song`")
...
async def SHUFFLE(message, message2, serverinfo, playerinfo):
    if len(serverinfo[message.guild].queue) > 1:
        ...
        if serverinfo[message.guild].queue[len(serverinfo[message.guild].queue) - 1][0].startswith("PLAYLIST: "):
            end = serverinfo[message.guild].queue[len(
                serverinfo[message.guild].queue) - 1]
        ...
        if index1 > 0 and index1 < len(serverinfo[message.guild].queue):
            if index2 > 0 and index2 < len(serverinfo[message.guild].queue):
                if not serverinfo[message.guild].queue[index1][0].startswith("PLAYLIST: "):
                    serverinfo[message.guild].queue.insert(
                        index2, serverinfo[message.guild].queue.pop(index1))
...
MUSC("!NEWPLAYLIST", "Creates a new music playlist of a given name\n!NEWPLAYLIST|name",
     NEWPLAYLIST, [str])
MUSC("!APPENDPLAYLIST", "Adds a song to a playlist corresponding to either entered query, link to a song (YouTube or Soundcloud), or link to a youtube playlist.\n!APPENDPLAYLIST|playlist name|query or link",
     APPENDPLAYLIST, [str, str])
MUSC("!DELETEPLAYLIST", "Deletes the music playlist of a given name\n!DELETEPLAYLIST|name",
     DELETEPLAYLIST, [str])
MUSC("!RENAMEPLAYLIST", "Renames a playlist\n!RENAMEPLAYLIST|original name|new name",
     RENAMEPLAYLIST, [str, str])
MUSC("!PLAYLISTS", "Displays a list of all playlists in the server\n!PLAYLISTS", PLAYLISTS, [])
MUSC("!MFIX", "This command will reset KIPP's voice client and related variables in order to fix most problems with music\n!MFIX", MFIX, [])
MUSC("!MUSIC", "This command will cause KIPP to play music from YouTube or Soundcloud in your voice channel based on your entered link or query\n!MUSIC|query or link",
     MUSIC, [str])
MUSC("!PAUSE", "This command will pause KIPP, if playing\n!PAUSE", PAUSE, [])
MUSC("!RESUME", "This command will resume KIPP, if paused\n!RESUME", RESUME, [])
MUSC("!REMOVESONG", "This command will remove the specified song from the queue\n!REMOVESONG|queue position",
     REMOVESONG, [int])
MUSC("!MOVESONG", "This command will move the song at the 'from index' to the 'to index'\n!MOVESONG|from position|to position",
     MOVESONG, [int, int])
MUSC("!CLEARQUEUE", "This command will clear all songs from the queue.\n!CLEARQUEUE", CLEARQUEUE, [])
MUSC("!SKIP", "This command will skip the current song, and play the next song in queue.\n!SKIP", SKIP, [])
MUSC("!QUEUE", "Displays a larger version of the queue list displayed in KIPP's music message.\n!QUEUE", QUEUE, [])
MUSC("!SHUFFLE", "Shuffles the queue.\n!SHUFFLE", SHUFFLE, [])
